...
//
// https://www.thingiverse.com/thing:1689741
...
label_size = 5;
label_thick = 0.8;
//label_rightside_of_teeth = false;
label_rightside_of_teeth = true;
...
module HandledComb() {
  flip_mirror = label_rightside_of_teeth ? [1, 0, 0] : [0, 0, 0];
  ...
  mirror(flip_mirror) {
    ...
    minkowski() {
      difference() {
        ...
        if (handle_hole_radius > 0) {
          translate([-handle_bot_width/2,
            handle_bot_width/2-handle_len, 0])
            cylinder(h=unsmoothed_thickness,
              r=handle_hole_radius+smooth_handle_by,
              $fn=24*handle_hole_radius);
        }
      }
    ...
    text_pos_y_end = (handle_hole_radius > 0) ?
      handle_bot_width/2 - handle_len + handle_hole_radius + smooth_handle_by :
